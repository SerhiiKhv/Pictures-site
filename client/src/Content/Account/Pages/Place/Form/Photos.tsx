...
export const Photos = ({photos, setPhotos}) => {
    ...
    function removePhoto(e, fileName) {
        e.preventDefault()
        return setPhotos([...photos.filter(photo => photo !== fileName)])
    }

    function mainPhoto(e, fileName) {
        e.preventDefault()
        return setPhotos([fileName, ...photos.filter(photo => photo !== fileName)])
    }
            ...
            {photos.length > 0 && photos.map(link => (
                <div className="h-32 flex relative" key={link}>
                    ...
                    <button onClick={e => removePhoto(e, link)}
                            className="cursor-pointer py-1 px-2 absolute bottom-1 right-1 bg-black text-white rounded-2xl bg-opacity-50">
                    ...
                    <button onClick={e => mainPhoto(e, link)}
                            className="cursor-pointer py-1 px-2 absolute bottom-1 left-1 bg-black text-white rounded-2xl bg-opacity-50">
